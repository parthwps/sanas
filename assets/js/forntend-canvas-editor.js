...
jQuery(document).ready(function ($) {
    ...
    $('#save-front-canvas-data').click(function () {
        var canvasData = canvas.toJSON();
        var cardId = $(this).attr('card-id');
        var eventId = $(this).attr('event-id'); // Ensure this attribute is correctly set
        var stepId = $(this).attr('step-id');
        var imageDataURL = canvas.toDataURL({
            format: 'png',
            quality: 1.0
        });
        var $backacardURL = $(this).attr('btn-url');
        ...
        if (redirectToNext) {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    security: $('#sanasfrontpagesecurity').val(),
